...
Source: https://github.com/KMnP/vpt/blob/main/src/models/vit_prompt/swin_transformer.py
"""
import math
import torch
...
from mmdet.models.backbones.swin_transformer import (
    BasicLayer, PatchMerging, SwinTransformerBase, SwinTransformerBlock,
...
class PromptedSwinTransformer(SwinTransformerBase):
    def __init__(
        self, prompt_location, prompt_tokens, prompt_dropout, 
        prompt_deep, prompt_project, prompt_init,
        pretrain_img_size=224, patch_size=4, in_channels=3,
        num_classes=1000, embed_dims=96, depths=[2, 2, 6, 2],
        num_heads=[3, 6, 12, 24], window_size=7, mlp_ratio=4., qkv_bias=True,
        qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
        norm_layer=nn.LayerNorm, use_abs_pos_embed=False, patch_norm=True,
        with_cp=False,  out_indices=(0, 1, 2, 3), **kwargs
    ):
        self.prompt_location = prompt_location
        if self.prompt_location == "pad":
            img_size += 2 * prompt_tokens
        super(PromptedSwinTransformer, self).__init__(
            pretrain_img_size, patch_size, in_channels, num_classes, embed_dims, depths,
            num_heads, window_size, mlp_ratio, qkv_bias, qk_scale, drop_rate,
            attn_drop_rate, drop_path_rate, norm_layer, use_abs_pos_embed, patch_norm,
            with_cp, **kwargs
        )
    ...
    def incorporate_prompt(self, x):
        ...
        elif self.prompt_location == "add":
            # add to the input patches + CLS
            # assert self.prompt_tokens == x.shape[1]
        ...
        elif self.prompt_location == "below":
